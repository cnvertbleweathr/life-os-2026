-- mart_cfb_preseason_quality_c.sql
--
-- Model C: "Talent pipeline predicts future quality"
-- Talent/program 60%, efficiency 25%, prior_results 15%
--
-- Hypothesis: recruiting ranking and NFL draft production predict NEXT
-- season better than last season's efficiency, because rosters turn over
-- and talent predicts replacement quality. The "buy low on bad teams
-- with elite recruiting" hypothesis -- a team that underperformed last
-- year but recruited well should bounce back. Compared against v1 in
-- phase_a_compare.py.
...
combined as (
    select
        ...
        pr.prior_results_z,
        -- Offense: talent 60% + efficiency 25% + prior_results 15%
        (coalesce(eff.efficiency_off_z, 0) * 0.25
       + coalesce(tal.talent_program_z, 0) * 0.60
       + coalesce(pr.prior_results_z, 0) * 0.15)
            / nullif(
                (case when eff.efficiency_off_z is not null then 0.25 else 0 end)
              + (case when tal.talent_program_z is not null then 0.60 else 0 end)
              + (case when pr.prior_results_z   is not null then 0.15 else 0 end), 0
            ) as preseason_off_rating_z,
        (coalesce(eff.efficiency_def_z, 0) * 0.25
       + coalesce(tal.talent_program_z, 0) * 0.60
       + coalesce(pr.prior_results_z, 0) * 0.15)
            / nullif(
                (case when eff.efficiency_def_z is not null then 0.25 else 0 end)
              + (case when tal.talent_program_z is not null then 0.60 else 0 end)
              + (case when pr.prior_results_z   is not null then 0.15 else 0 end), 0
