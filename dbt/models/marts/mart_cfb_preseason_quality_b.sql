-- mart_cfb_preseason_quality_b.sql
--
-- Model B: "Recent on-field performance is what matters"
-- Efficiency 70%, talent/program 20%, prior_results 10%
--
-- Hypothesis: what a team DID last season on the field (PPA, success
-- rate, havoc) is more predictive than recruiting or win-loss record,
-- because those lag actual quality. Useful for identifying teams that
-- over- or under-recruit relative to real on-field performance.
-- Compared against v1 (equal weights) in phase_a_compare.py.
...
combined as (
    select
        ...
        pr.prior_results_z,
        -- Offense: efficiency 70% + talent 20% + prior_results 10%
        (coalesce(eff.efficiency_off_z, 0) * 0.70
       + coalesce(tal.talent_program_z, 0) * 0.20
       + coalesce(pr.prior_results_z, 0) * 0.10)
            / nullif(
                (case when eff.efficiency_off_z is not null then 0.70 else 0 end)
              + (case when tal.talent_program_z is not null then 0.20 else 0 end)
              + (case when pr.prior_results_z   is not null then 0.10 else 0 end), 0
            ) as preseason_off_rating_z,
        (coalesce(eff.efficiency_def_z, 0) * 0.70
       + coalesce(tal.talent_program_z, 0) * 0.20
       + coalesce(pr.prior_results_z, 0) * 0.10)
            / nullif(
                (case when eff.efficiency_def_z is not null then 0.70 else 0 end)
              + (case when tal.talent_program_z is not null then 0.20 else 0 end)
              + (case when pr.prior_results_z   is not null then 0.10 else 0 end), 0
