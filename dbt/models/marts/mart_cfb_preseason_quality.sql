-- mart_cfb_preseason_quality.sql
--
-- Phase A of the Quality-of-Win/SOS system (design v5,
-- docs/cfb_quality/QUALITY_OF_WIN_DESIGN.md). Static preseason quality
-- score per (team, season) — computed once, never updated in-season.
--
-- Three blocks (efficiency, talent_and_program, prior_results), averaged
-- within each block first, then combined with EQUAL block weight (1/3
-- each) — the locked v1 baseline per design v5, not tuned against ATS
-- results, not chosen by inspection. A ridge-regression alternative is
-- the designated SECOND benchmark in Stage 1 validation, never a
-- replacement for this baseline without evidence.
--
-- Grain: one row per (team, season). Uses PRIOR season's data only
-- (e.g. season=2024's quality score is built from 2023 stats) -- same
-- no-lookahead discipline as the rest of this codebase.
--
-- CRITICAL: def_ppa and def_success_rate are SIGN-REVERSED before
-- z-scoring. Higher def_ppa/def_success_rate ALLOWED is worse defense
-- -- verified directly against first principles before this design was
-- adopted (see design v5 changelog). def_havoc_total is NOT reversed;
-- higher havoc is genuinely better defense.
--
-- Percentile-to-z conversion uses dbt/macros/inv_normal_cdf.sql --
-- DuckDB has no built-in inverse normal CDF (confirmed by direct
-- testing: erf_inv/norm_inv/inverse_normal_cdf/normal_inv/erf all fail).
-- Fed p = (rank - 0.5) / n (rank-based, never exactly 0 or 1) per design
-- v5's correction, avoiding infinite z-values at the percentile extremes.
...
combined as (
    select
        ...
        pr.prior_results_z,
        (coalesce(eff.efficiency_off_z, 0) + coalesce(tal.talent_program_z, 0) + coalesce(pr.prior_results_z, 0))
            / nullif(
                (case when eff.efficiency_off_z is not null then 1 else 0 end)
              + (case when tal.talent_program_z is not null then 1 else 0 end)
              + (case when pr.prior_results_z   is not null then 1 else 0 end), 0
            ) as preseason_off_rating_z,
        (coalesce(eff.efficiency_def_z, 0) + coalesce(tal.talent_program_z, 0) + coalesce(pr.prior_results_z, 0))
            / nullif(
                (case when eff.efficiency_def_z is not null then 1 else 0 end)
              + (case when tal.talent_program_z is not null then 1 else 0 end)
              + (case when pr.prior_results_z   is not null then 1 else 0 end), 0
