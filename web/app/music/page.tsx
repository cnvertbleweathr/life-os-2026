...
import { musicApi, type MusicSummary, type MusicDaily10, type TopArtist, type TopTrack, type NewsArticle } from "@/lib/api";
import {
...
export default function MusicPage() {
  ...
  useEffect(() => {
    ...
    // Separate call/catch from the rest — a news-fetch failure
    // shouldn't block the whole page, same pattern as the Sports page.
    musicApi.news()
      .then(setNews)
  ...
  return (
    <div className="ons-page" style={{ padding: "34px 44px 48px", maxWidth: 1380, margin: "0 auto" }}>
      ...
      <div className="grid gap-[22px] items-start" style={{ gridTemplateColumns: "1.3fr 1fr" }}>
        {/* Left: cover + Spotify player */}
        <Card accent pad={0} style={{ overflow: "hidden" }}>
          ...
          {daily10.available && daily10.playlist_id && (
            <iframe
              src={`https://open.spotify.com/embed/playlist/${daily10.playlist_id}?utm_source=generator`}
              width="100%"
              height="660"
              style={{ border: "none", borderBottom: "1px solid #ebe5d8", display: "block" }}
              allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"
              loading="lazy"
              title="Today's Daily 10 playlist"
            />
          )}
        ...
        {/* Right: Music News */}
        <Card pad={0}>
          <div style={{ padding: "16px 18px 12px" }}><K>Music News</K></div>
      ...
      {/* Bottom: Top Artists + Most-Played Tracks side by side */}
      <div className="grid gap-[22px] items-start mt-[22px]" style={{ gridTemplateColumns: "1fr 1fr" }}>
        <Card pad={0}>
